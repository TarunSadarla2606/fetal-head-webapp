        ...
        {studies.map(study => (
          <li key={study.id}>
            <button
              ...
              className={cn(
                'w-full text-left px-3 py-2.5 flex items-start gap-2.5 hover:bg-slate-800/50 transition-colors',
                selectedId === study.id && 'bg-[#0D7680]/10 border-l-2 border-[#0D7680] pl-[10px]'
              )}
            >
              <User className="w-3.5 h-3.5 mt-0.5 text-slate-500 shrink-0" />
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-1">
                  <span className="text-sm font-medium text-slate-200 truncate">{study.patientName}</span>
                  {statusIcon[study.status]}
                </div>
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className="text-xs text-slate-500">{study.studyDate}</span>
                  {study.isDemo && (
                    <span className="text-[9px] font-bold px-1 py-px bg-[#0D7680]/20 text-[#0D7680] rounded uppercase tracking-wide">
                      demo
                    </span>
                  )}
                </div>
