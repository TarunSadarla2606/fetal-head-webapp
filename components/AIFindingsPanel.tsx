          ...
          {f && (
            <>
              ...
              {f.mode === 'cine_clip' && (f.cine_overlay_gif || f.cine_loop_gif) && (
                <p
                  data-testid="cine-panel-pointer"
                  className="flex items-start gap-1.5 text-[10px] text-slate-500 leading-snug"
                >
                  <Film className="w-3 h-3 shrink-0 mt-0.5 text-[#0D7680]" />
                  <span>
                    Cine loop{f.cine_frame_count ? ` (${f.cine_frame_count} frames)` : ''} —
                    open the <span className="text-slate-300 font-medium">Cine Loop</span> tab
                    to view the animation full size.
                  </span>
                </p>
              )}
